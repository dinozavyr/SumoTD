...
class TrafficDemand:
    ...
    def CallDuaRouter(self):
        cmd = 'duarouter -c {output_path}'.format(
            output_path=Path.cwd() / Path('simulation/Data/Out/duaCFG.xml'),
        )
        subprocess.run(cmd, shell=True)
                    ...
                    for e in range(0, len(emergencyDecel)):
                        for s in range(0, len(sigma)):
                            for t in range(0, len(tau)):
                                line = '\t<vType id="vdist1" vClass="passenger" color="1,0,0" carFollowModel="Krauss"' + ' minGap="' + str(
                                    minGap[m]) + '" accel="' + str(accel[a]) + '" decel="' + str(
                                    decel[d]) + '" emergencyDecel="' + str(
                                    emergencyDecel[e] + decel[d]) + '" sigma="' + str(sigma[s]) + '" tau="' + str(
                                    tau[t]) + '" />' + '\n'
    ...
    def CalculateKraussOrig1(self, opath, i):
        ...
        for m in minGap:
            for t in tau:
                line = '\t<vType id="vdist1" vClass="passenger" color="1,0,0" carFollowModel="KraussOrig1"' + ' minGap="' + str(
                    m) + '" tau="' + str(t) + '"/>' + '\n'
                df = self.CreateFlowFile(i, line)
    ...
    def CalculatePWagner2009(self, ofile, i):
        ...
        for m in range(0, len(minGap)):
            for t in range(0, len(tau)):
                line = '<vType id="vdist1" vClass="passenger" color="1,0,0" carFollowModel="PWagner2009"' + ' minGap="' + str(
                    minGap[m]) + '" tau="' + str(tau[t]) + '"/>' + '\n'
                df = self.CreateFlowFile(i, line)
                result.append(df, ignore_index=True)
        result.to_csv(ofile)
        ...
        for m in range(0, len(minGap)):
            for s in range(0, len(security)):
                for e in range(0, len(estimation)):
                    for t in range(0, len(tau)):
                        line = '/t<vType id="vdist1" vClass="passenger" color="1,0,0" carFollowModel="Wiedemann"' + ' minGap="' + str(
                            minGap[m]) + '" security="' + str(security[s]) + '" estimation="' + str(
                            estimation[e]) + '" tau="' + str(tau[t]) + '"/>' + '\n'
                        df = self.CreateFlowFile(i, line)
                        result.append(df, ignore_index=True)
        result.to_csv(ofile)
